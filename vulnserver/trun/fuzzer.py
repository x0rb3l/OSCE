...
def main():
	...
	s_initialize("TRUN")                             # This just gives the session the name, "TRUN"
	
	s_string("TRUN", fuzzable = False)     # strings are fuzzable by default, so here I set it to 'false'
	...
	session.connect(s_get("TRUN"), callback=get_banner)      # Connect to the server following the guidlines defined above. Set the callback function.
